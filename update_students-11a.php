<?php
	...
	$select_student = mysql_fetch_array(mysql_query("SELECT * FROM fls1egvn_students WHERE student_nr = 1"));
	echo "$select_student[student_nr] $select_student[class] $select_student[name] $select_student[surname] $select_student[family]<br />";
	...
	{$check = FALSE;
		...
		$student = fgets($students, 255);
			...
			$st1 = $select_student['name']. " ". $select_student['surname']. " ".$select_student['family'];
			//За всяко име от БД проверяваме дали съвпада с прочетеното от файла
			$check = preg_match("/$st1/", "$student");
			$check1 = preg_match("/$select_student[class]/", "$student");
			if ($check && $check1) break;
		
	}if ($check1) echo $student." $check1<br />$select_student[class]"; else echo "$check1 Error!";
